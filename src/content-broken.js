// content.js - Content script with fetch patching and overlay UI (Fixed version)
...
(async () => {
  ...
  // Inline core functions (to avoid external script loading issues)
  ...
  // Simple tokenizer (4:1 char-to-token approximation)
  ...
  function computeCostUSD(model, inTok, outTok, pricingTable = DEFAULT_PRICING) {
    const pricing = pricingTable.find(p => model?.startsWith(p.modelPrefix)) || DEFAULT_PRICING[0];
  ...
  function extractDeltaText(evt) {
    ...
    if (evt.message?.content?.parts) return evt.message.content.parts.join('');
    if (evt.choices?.[0]?.delta?.content) return evt.choices[0].delta.content;
    if (evt.choices?.[0]?.text) return evt.choices[0].text;
    return null;
  }
  ...
  function buildInitialMetrics(reqBodyText, url, t0) {
    let model = 'gpt-4';
    let messages = [];
    let temperature, max_tokens, requestId, conversationId;
    ...
    return {
      ...
      requestId, conversationId, model,
      promptTokens, completionTokens: 0,
      inputCostUSD: 0, outputCostUSD: 0, totalCostUSD: 0,
      t0: Date.now(), tTFB: null, tDone: null,
      latencyMs: null, ttfbMs: null,
  ...
  async function init() {
    ...
    if (response && response.ok) {
      ...
      pricingTable = settings.pricing || DEFAULT_PRICING;
    } else {
  ...
  function patchFetch() {
    ...
    window.fetch = async function (input, init) {
      ...
      console.log('[Revenium] Intercepted chat request');
      ...
      (async () => {
        ...
        let metrics = buildInitialMetrics(reqBodyText, url, t0);
        ...
        try {
          ...
          finalizeMetrics(metrics, totalAssistantText, pricingTable);
